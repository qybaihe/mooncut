...
### 5.6 `GET /v1/capabilities` · `GET /v1/me/capability-installations` · `GET /v1/community*`
...
社区 / 能力安装的完整实现在 **本机 Agent**，但生产边缘 **未把这些路径全部列入 Tunnel 反代白名单**。  
**App 一期请勿依赖能力市场 / 社区发帖**，除非产品确认已打开反代。

Web 公开社区能力包另有 **EdgeOne 静态 Registry**（与本 API 分离，见 `communityRegistry` 服务）。
