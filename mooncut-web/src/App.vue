...
import type { AuthUser, BillingPlanId, PetAnimationState, VideoAsset, WorkspacePage } from './types'
...
const accountUpgradePlan = ref<Exclude<BillingPlanId, 'free'> | null>(null)
...
const activePetState = computed(() => {
  ...
  if (activePage.value === 'studio') return 'idle'
...
watch(activePage, (page) => {
  if (page === 'edit' || page === 'record' || page === 'me') {
...
function openBilling(plan: Exclude<BillingPlanId, 'free'>) {
  accountUpgradePlan.value = plan
  activePage.value = 'me'
}
...
<template>
  ...
  <div
    ...
    :class="{
      ...
      'is-studio': activePage === 'studio',
      'is-pricing': activePage === 'pricing',
      'is-privacy': activePage === 'privacy',
    }"
  >
    <AppNavigation
      v-if="authUser && activePage !== 'landing' && activePage !== 'studio' && activePage !== 'pricing' && activePage !== 'privacy'"
      v-model="activePage"
      :immersive="immersive"
    ...
    <main
      ...
      :class="{
        'landing-main':
          activePage === 'landing' || activePage === 'studio' || activePage === 'pricing' || activePage === 'privacy',
      }"
    >
      <LandingPage
      ...
      <template v-if="authUser">
        ...
        <MeStudio
          ...
          :user="authUser"
          :upgrade-plan="accountUpgradePlan"
          @logout="handleLogout"
          @open-community="activePage = 'public-community'"
          @open-privacy="activePage = 'privacy'"
          @open-pricing="activePage = 'pricing'"
    ...
    <PetCompanion
      ...
      :message="petMessage"
      :immersive="immersive"
      :landing="activePage === 'landing' || activePage === 'studio' || activePage === 'pricing' || activePage === 'privacy'"
    />
  </div>
</template>
