...
| `codex` | **Codex 非交互式 Agent**（`gpt-5.6-terra` + `xhigh`）+ 同一套 MoonCut tools 与 ImageGen 导入桥接 |
...
Codex 模式说明见 [docs/CODEX_AGENT.md](./docs/CODEX_AGENT.md)。它把网页的既有异步任务协议接到受限的 Codex `exec`，创作文件仅可写入当前任务和 Remotion 媒体目录：

```bash
MOONCUT_AGENT_EXECUTION_MODE=codex \
MOONCUT_CODEX_MODEL=gpt-5.6-terra \
MOONCUT_CODEX_REASONING_EFFORT=xhigh \
npm run edit -- /path/to/video.mp4 "按 SPEC 完整剪辑"
```
...
`evidence` 现在也支持 AI 自主的多来源编排：默认仍是单窗；只有两到三份来源各自提供不同信息时，才通过 `evidencePanels` 选择 `parallel`、`comparison` 或 `sequence`，并在独立 16:9 Safari 窗口中分别滚动。重复 ID、URL、用途和未标明的冲突会在渲染前被质量门禁拒绝。完整协议见 [`docs/MULTI_EVIDENCE_VISUAL_SPEC.md`](docs/MULTI_EVIDENCE_VISUAL_SPEC.md)。

概念页不再固定为同一组大按钮/数字：`desktop` beat 可按语义选择 `editorial`、`workflow`、`comparison`、`dashboard` 四种 16:9 模板。Codex 还可调用本机 Excalidraw Skill，保留可编辑 JSON 和 PNG，经 `import_handdrawn_diagram` 登记后用于独立 `diagram` beat；它只负责解释结构，不会进入真实证据集合。
