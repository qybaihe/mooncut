...
### 启用 GitHub Actions（一次性）

工作流模板路径：[`docs/ci/ios-ipa.yml`](./ci/ios-ipa.yml)。

当前推送所用 OAuth token 无 `workflow` scope，无法直接写入 `.github/workflows/`。仓库管理员请任选其一：

```bash
# A) 本机用带 workflow 权限的 token / SSH
mkdir -p .github/workflows
cp docs/ci/ios-ipa.yml .github/workflows/ios-ipa.yml
git add .github/workflows/ios-ipa.yml
git commit -m "ci: enable public iOS IPA workflow"
git push

# B) GitHub 网页：Actions → New workflow → 粘贴 docs/ci/ios-ipa.yml 内容
```

启用后：

1. Actions → **iOS Public IPA** → Run workflow  
2. 产物 Artifact：`MoonCut-public-unsigned-ipa`  
3. 打 tag `ios-v*` 时也可挂到 Release 附件  

**安装提醒**：unsigned IPA 不能直接点装；需用户用自己的开发者证书重签。App 内会标明「公开预览包 · 服务未配置」。
