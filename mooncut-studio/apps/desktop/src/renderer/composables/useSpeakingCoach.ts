...
export function useSpeakingCoach(script: Ref<string>) {
  ...
  async function startVision(video: HTMLVideoElement | null) {
    if (!video) return
    try {
      const vision = await FilesetResolver.forVisionTasks(publicAsset('mediapipe'))
      const options = {
        baseOptions: { modelAssetPath: publicAsset('models/face_landmarker.task'), delegate: 'GPU' as const },
